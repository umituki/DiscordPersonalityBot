...
ClaimKind = Literal[
    ...
    #: Who she is, rather than what she has done: her name, her age, when she
    #: was born. Settled by the identity anchors, which is the only place those
    #: are decided — and never by the model reasoning about them.
    "yui_identity_fact",
...
EvidenceKind = Literal[
    #: Her name, her birthday, her age today. Derived in Python from the life
    #: anchors, so an age is a calculation rather than a recollection.
    "identity_fact",
    "activity",
    "objective_event",
    "subjective_memory",
...
ACCEPTED_EVIDENCE: dict[ClaimKind, tuple[EvidenceKind, ...]] = {
    ...
    # Nothing else. An identity fact is not remembered, observed or inferred —
    # it is read off the anchors, and if the anchors are not there the honest
    # answer is that she does not know.
    "yui_identity_fact": ("identity_fact",),
...
class GroundingContext:
    ...
    #: Who she is, from the life anchors. Empty before FIRST BOOT has settled
    #: them, which is why an ungrounded age question is answered with "I do not
    #: know" rather than with a number.
    identity_facts: tuple[Evidence, ...] = ()
